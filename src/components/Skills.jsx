      ...
      <div className="max-w-6xl mx-auto text-center">
        ...
        {categories.map((cat) => (
          <div key={cat.title} className="mb-12" data-aos="fade-up" data-aos-delay="100">
            <h3 className="font-bold text-gray-800 mb-4 text-lg md:text-xl">{cat.title}</h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-8 sm:gap-10 justify-items-center">
              {cat.items.map((item) => (
                <div
                  key={item.name}
                  className="w-full aspect-square flex flex-col items-center justify-center space-y-3 rounded-lg border-2 shadow-sm hover:shadow-md transition bg-white/70 backdrop-blur p-6"
                >
